...
class Player(Entity):
    ...
    def update(self, keys, mouse_buttons, platforms, enemies):
        ...
        # Jump / Fly in Dev Mode
        import settings
        if keys[pygame.K_w] or keys[pygame.K_UP] or keys[pygame.K_SPACE]:
            if settings.DEV_MODE:
                self.vel_y = -8 # Constant upwards flight
                self.on_ground = False
            elif self.on_ground:
    ...
    def draw(self, surface, camera_offset_x):
        ...
        # Touches the grass top perfectly (0 offset)
        draw_rect.y += 0
...
class Monkey(Entity):
    ...
    def draw(self, surface, camera_offset_x):
        ...
        if current_time - self.last_attack_time < 300:
            ...
            # Center and align base: sprite is 54px high, hitbox is 40px high. Diff = 14px.
            draw_rect.x -= 7
            draw_rect.y += 14
        elif self.vel_x != 0:
            ...
            # Center and align base: sprite is 50px high, hitbox is 40px high. Diff = 10px.
            draw_rect.x -= 5
            draw_rect.y += 10
        else:
...
class Bird(Entity):
    ...
    def draw(self, surface, camera_offset_x):
        # Draw 40x40 sprite centered over 30x30 hitbox
        draw_rect = pygame.Rect(self.rect.x - 5 - camera_offset_x, self.rect.y - 5, 40, 40)
